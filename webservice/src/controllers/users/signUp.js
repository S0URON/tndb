  ...
  try {
    ...
    if (emailUsed)
      return res
        ...
        .json({ message: "email_already_used" });

    const newUserId = new mongoose.Types.ObjectId();
    const profilePic = req.files.profilePicture[0];
    const coverPic = req.files.coverPicture[0];

    const coverPicturePath = `static/users/cover/${newUserId}.${
      coverPic.mimetype.split("/")[1]
    }`;
    const profilePicturePath = `static/users/profile/${newUserId}.${
      profilePic.mimetype.split("/")[1]
    }`;
    const profileTempPath = `static/${profilePic.originalname.split(".")[0]}.${
      profilePic.mimetype.split("/")[1]
    }`;
    const coverTempPath = `static/${coverPic.originalname.split(".")[0]}.${
      coverPic.mimetype.split("/")[1]
    }`;
    ...
    const newUser = {
      ...
      watchList: [],
      coverPicture: coverTempPath,
      profilePicture: profileTempPath,
      token: "",
      createdAt: new Date(),
    };
